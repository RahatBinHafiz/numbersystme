...
export function BinaryGrouping({ data, activeStep }: Props) {
  ...
  const padded = data.paddedInput !== data.input
  const padCount = data.paddedInput.length - data.input.length

  return (
    <div className="space-y-6">
      <div>
        <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-[var(--ink-soft)]">
          Group Binary Digits into {data.groupSize}-bit clusters (Right to Left)
        </p>
        {padded && (
          <div className="mb-3 rounded-xl p-3 border text-xs text-[var(--ink-soft)] bg-[color-mix(in_srgb,var(--surface)_50%,transparent)]" style={{ borderColor: 'rgb(var(--border) / var(--border-alpha))' }}>
            <span className="font-semibold text-[var(--ink)]">Zero-padding applied:</span> The binary number was padded on the left with {padCount} zero{padCount > 1 ? 's' : ''} to complete {data.groups.length} full {data.groupSize}-bit group{data.groups.length > 1 ? 's' : ''}.
