...
export function DivisionTable({ data, activeStep }: Props) {
  ...
  const visible = data.steps.slice(0, activeStep + 1)
    ...
    <div className="space-y-6">
      <div>
        <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
          <p className="text-xs font-semibold uppercase tracking-wide text-[var(--ink-soft)]">
            Repeated Integer Division by {data.toBase} ({toStyle.name})
          </p>
          <span className="text-xs text-[var(--ink-soft)]">
            Step {Math.min(activeStep + 1, data.steps.length)} of {data.steps.length}
          </span>
        </div>
      ...
      <div className="rounded-2xl p-4 border bg-[color-mix(in_srgb,var(--surface)_70%,transparent)]" style={{ borderColor: 'rgb(var(--border) / var(--border-alpha))' }}>
        <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-[var(--ink-soft)]">
          Read Remainders: Bottom → Top (MSB to LSB)
        </p>
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-xs text-[var(--ink-soft)] font-medium">Order:</span>
          {visible
            .slice()
            .reverse()
            .map((step, i) => (
              <motion.span
                ...
                className="font-mono-num flex h-10 w-10 items-center justify-center rounded-xl border text-lg font-bold shadow-sm"
                style={{
